# 6iBrief Worträtsel – Setup-Anleitung
...
**Gute Nachricht: Das meiste ist schon erledigt.** Die gleiche Notion-Integration
("Tsüri Worträtsel", oder wie du sie genannt hast) ist bereits mit der
Datenbank **"ADMIN alle Einträge"** verbunden – dort liegen ja auch die
6iBrief-Einträge drin, du brauchst also **keine neue Notion-Integration** zu
erstellen.

Du brauchst nur die **gleichen zwei Werte** wie beim Tsüri-Worträtsel-Projekt,
diesmal als Secrets im *neuen* Worker (6iBrief):
- `NOTION_TOKEN` – dasselbe Internal Integration Secret wie vorher
- `NOTION_DATABASE_ID` – dieselbe ID: `28ce29f5-f081-80b0-8c89-e01d1704dd87`

**So bestimmst du das Wort des Tages:** In der Datenbank hat jeder Eintrag mit
`Datenbanktyp = 6iBrief` ein Feld **"Wordle"**. Trage dort für den
...
`https://6ibrief-wortraetsel.<dein-account>.workers.dev`
...
```html
<iframe
  src="https://6ibrief-wortraetsel.<dein-account>.workers.dev"
  width="100%"
  height="640"
  style="border:none; max-width:460px;"
  title="6iBrief Worträtsel"
></iframe>
```
...
`https://6ibrief-wortraetsel.<dein-konto>.workers.dev/api/stats`

Oder noch einfacher – eine fertige Übersichtsseite statt rohem JSON:
`https://6ibrief-wortraetsel.<dein-konto>.workers.dev/stats.html`
...
1. Gehe zu https://api.slack.com/apps → **Create New App** → **From scratch**
   → Namen geben (z.B. "6iBrief Worträtsel Alerts" – oder denselben Slack-Webhook
   wie beim Tsüri Worträtsel wiederverwenden, dann diesen Schritt überspringen)
   → Workspace auswählen.
