...
// "ADMIN alle Einträge": nimmt den neuesten 6iBrief-Eintrag (Datum <= heute),
...
async function getActiveWord(env, ctx) {
  ...
  try {
    const res = await fetch(
      `https://api.notion.com/v1/databases/${env.NOTION_DATABASE_ID}/query`,
      {
        ...
        body: JSON.stringify({
          filter: {
            and: [
              { property: "Datenbanktyp", select: { equals: "6iBrief" } },
    ...
    if (!page) {
      throw new Error(
        "Kein 6iBrief-Eintrag mit ausgefülltem Wordle-Feld gefunden (Datum <= heute)."
      );
    }
